...
void driveForward2(int motorSpeedLeft,int motorSpeedRight){
  
  analogWrite(RPWM_Output, 0);
  analogWrite(LPWM_Output, motorSpeedLeft);

  analogWrite(RPWM_Output3, motorSpeedRight);
  analogWrite(LPWM_Output3, 0);
}
...
void driveBackward2(int motorSpeedLeft,int motorSpeedRight){
  
  analogWrite(RPWM_Output, motorSpeedLeft);
  analogWrite(LPWM_Output, 0);

  analogWrite(RPWM_Output3, 0);
  analogWrite(LPWM_Output3, motorSpeedRight);
}
...
void drive(int forBackWardChannel, int leftRightChannel){
  if(PWM[leftRightChannel] > 50 || PWM[leftRightChannel] < -50){
    if(PWM[leftRightChannel] >= 50){
      Serial.print("right");
      driveLeft(PWM[leftRightChannel]);
    }else if(PWM[leftRightChannel] < -50){
      Serial.print("left");
      driveRight((-1)*PWM[leftRightChannel]);
    }else{
      stopDrive();
    }
  }
    ...
    if(PWM[forBackWardChannel] >= 10){
      ...
      driveForward(PWM[forBackWardChannel]);
    }else if(PWM[forBackWardChannel] < -10){
      Serial.print("backward");
      driveBackward((-1)*PWM[forBackWardChannel]);
    }else{
      stopDrive();
    }
  }
