...
void expand(int pwmSignal){
  digitalWrite(in1, LOW);
  digitalWrite(in2, HIGH);
  analogWrite(enA, pwmSignal);
}

void shrink(int pwmSignal){
...
void adjustClaw(){  //CLAW will be set by channel 4
  if(PWM[RC_CH4] > 10 || PWM[RC_CH4] < -10){
    if(PWM[RC_CH4] >= 50){
      Serial.print("expand");
      expand(PWM[RC_CH4]);
    }else if(PWM[RC_CH4] <= -50){
      Serial.print("shrink");
      shrink((-1)*PWM[RC_CH4]);
    }else{
      stopClaw();
